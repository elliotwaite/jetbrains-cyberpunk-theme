...
def get_theme_color_map(theme_path):
  ...
  """
    ...
    return {name: hex_str_to_hex_value(hex_str) for name, hex_str in theme['colors'].items()}
...
def replace_option_value_in_line(line, color_map, ignore_missed_matches=False):
    ...
    return line.replace(f' value="{option_value}" ', f' value="{color_map[option_value]}" ')
...
def build_color_scheme(source_path, output_path, theme_path, file_statuses_path):
    ...
    input_lines = (
        input_lines[:end_of_colors_index] + file_status_lines + input_lines[end_of_colors_index:]
    )
